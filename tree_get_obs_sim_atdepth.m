   ...
   % Date: 31.07.2014
function [similarNodes] = tree_get_obs_sim_atdepth(tree, depth, height, observations, mt)  
    ...
    %{
    similarNodes(1:noOfObservations+1:noOfObservations*noOfObservations) = 1;
    obsnodes = M;
    while ~isempty(find(obsnodes, 1))
        nonzeros = find(obsnodes);
        eqIndices = find( M == obsnodes(nonzeros(1)) );
        k = eqIndices(1);
        for m = 1:size(eqIndices,2)
            similarNodes(k,eqIndices(m)) = 1;
        end
        obsnodes(eqIndices) = 0;
    end
    %}
